    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="card p-5 rounded-2xl h-44 animate-pulse bg-slate-200 dark:bg-slate-800" />
          ))}
        </div>
      ) : (!Array.isArray(wards) || wards.length === 0) ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-[#0B132B]">
          <p className="text-xs text-slate-400 font-medium">No ward or zone scorecards recorded for this period.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {wards.map(w => (
            <div key={w.id || w.wardNumber} className="card p-5 rounded-2xl space-y-3.5 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between">
                <div>
                  <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300">
                    WARD {w.wardNumber || 'N/A'}
                  </span>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white mt-1">{w.name || 'Unnamed Ward'}</h3>
                </div>
                <div className="text-right">
                  <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">{w.resolutionRate || 0}%</span>
              ...
              <div className="grid grid-cols-3 gap-2 text-center bg-slate-50 dark:bg-slate-800/60 p-2.5 rounded-xl text-xs">
                <div>
                  ...
                  <strong className="text-slate-800 dark:text-white text-sm">{w.totalComplaints || 0}</strong>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-semibold block">RESOLVED</span>
                  <strong className="text-emerald-600 text-sm">{w.resolved || 0}</strong>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-semibold block">SLA</span>
                  <strong className="text-teal-600 text-sm">{w.slaCompliance || 0}%</strong>
                </div>
              </div>
              ...
              <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300">
                <div className="flex items-center justify-between">
                  ...
                  <strong className="capitalize text-slate-800 dark:text-white">{w.topCategory || 'None'}</strong>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-400">Active AI Hotspots:</span>
                  <strong className={w.hotspotCount > 0 ? 'text-amber-600 font-bold' : ''}>{w.hotspotCount || 0}</strong>
                </div>
              </div>
